...
// Defining the Kafka server parameters
    ...
    val topics = Array("cs.bigdata-streams") //topics list
    ...
    ssc.checkpoint("hdfs://0.0.0.0:8020/TwitCount_checkpoint")
    val cnt = splits.map(x => (x, 1)).reduceByKey(_ + _).updateStateByKey(updateFunc)
    def toHBase(row: (_, _)) {
      val hConf = new HBaseConfiguration()
      hConf.set("hbase.zookeeper.quorum", "0.0.0.0:2182")
      val tableName = "Streaming_Twittercount"
      ...
      thePut.add(Bytes.toBytes("Twit_count"), Bytes.toBytes("Occurances"), Bytes.toBytes(row._2.toString))
      hTable.put(thePut)
    }
